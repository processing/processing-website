...
Interactive systems that &ldquo;listen&rdquo; to an audio input typically use a few simple techniques to abstract a complex sound source into a control source that can be mapped as a parameter in interaction design. For example, a plot of average amplitude of an audio signal over time can be used to modulate a variable continuously through a technique called *envelope following*. Similarly, a threshold of amplitude can be set to trigger an event when the sound reaches a certain level; this technique of *attack detection* (&ldquo;attack&rdquo; is a common term for the onset of a sound) can be used, for example, to create a visual action synchronized with percussive sounds coming into the computer.
...
[^3]:John Cage, &ldquo;Credo: The Future of Music (1937),&rdquo; in *John Cage: An Anthology*, edited by Richard Kostelanetz (Praeger, 1970),  p. 52.
