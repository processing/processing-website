...
With the release of Processing 4.0, the _FX2D_ renderer is no longer included. Use it for fast 2D graphics on large and high resolution displays for more speed than the default renderer. This renderer is still experimental, but it useful for certain conditions. It can be downloaded as a Library.
...
The next example draws four circles on the screen and utilizes a custom function called `twoCircles()`. The `twoCircles()` function is not a part of the Processing language, but was written for this example. The code in `draw()` only runs once because `noLoop()` is called in `setup()`.</p>
