...
import * as css from './NavbarItems.module.css';
...
export const items = [
    {
        ...
        children: [
            ...
            { name: 'raspberry-pi', href: '/download/raspberry-pi' }
        ]
    },
    {
        name: 'documentation',
...
function NavbarElement({ className }) {
    ...
    const classes = classnames({
        [css.item]: true,
        [css.hasSubmenu]: children && !hideChildren,
...
function NavbarItemChildren() {
    const { children, hideChildren } = useContext(NavbarItemContext);
        ...
        <ul className={css.submenu}>
            {children.map((subitem) => (
                <NavbarItemContext.Provider value={subitem}>
                    <NavbarElement className={css.subitem} />
                </NavbarItemContext.Provider>
            ))}
        </ul>
    )
